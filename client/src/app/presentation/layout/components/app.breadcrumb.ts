...
import { BreadcrumbHelper } from '@/core/helpers/breadcrumb.helper';
...
export class AppBreadcrumb {
    ...
    constructor(private router: Router, private breadcrumbHelper: BreadcrumbHelper) {
        this.router.events.pipe(filter((event) => event instanceof NavigationEnd)).subscribe((event) => {
            const root = this.router.routerState.snapshot.root;
            const breadcrumbs: Breadcrumb[] = [];
            this.addBreadcrumb(root, [], breadcrumbs);
            this.breadcrumbHelper.customLabel$.subscribe(label => {
                if (label && breadcrumbs.length > 0) {
